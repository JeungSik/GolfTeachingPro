...
import kr.co.anitex.golfteachingpro.observablescrollview.Scrollable

/**
 * Created by nitinagarwal on 11/1/15.
 */
class ObservableExpandableListView : ExpandableListView, Scrollable {
        ...
        if (mCallbacks != null) {
            if (childCount > 0) {
                ...
                if (firstVisibleChild != null) {
                    ...
                    currentScrollY = mPrevScrolledChildrenHeight - firstVisibleChild.top
                    mPrevFirstVisiblePosition = firstVisiblePosition
                    mCallbacks!!.onScrollChanged(currentScrollY, mFirstScroll, mDragging)
                    if (mFirstScroll) {
                        mFirstScroll = false
                    }
                    mScrollState = when {
